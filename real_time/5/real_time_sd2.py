...
mus = np.concatenate([np.linspace(0,0.01,11), np.linspace(0.01,0.1,11)[1:]])
...
etas = [0.3,0.7,0.5,0.8,0.9,0.95]

i_eta = np.repeat(np.arange(len(etas)), len(mus))
i_mu = np.tile(np.arange(len(mus)), len(etas))
...
for i in np.arange(len(i_eta))[task_ID::num_tasks]:
    eta = etas[i_eta[i]]
    mu = mus[i_mu[i]]
    ...
    sel2 = np.logical_and(t>=0, t*w_re<20*np.pi)
    if mu == 0:
